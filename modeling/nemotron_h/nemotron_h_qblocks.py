...
class W4A16NemotronHMamba2Mixer(nn.Module):
    """
...
class W4A16NemotronHMLP(nn.Module):
...
class W4A16NemotronHAttention(nn.Module):
...
class W4A16NemotronHBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        if self.block_type == "mamba":
            self.mixer = W4A16NemotronHMamba2Mixer(config, layer_idx=layer_idx)
        elif self.block_type == "attention":
            self.mixer = W4A16NemotronHAttention(config, layer_idx=layer_idx)
        elif self.block_type == "mlp":
            self.mixer = W4A16NemotronHMLP(config, layer_idx=layer_idx)
        else:
