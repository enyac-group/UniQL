...
from .qwen2_blocks import Qwen2DecoderLayer
from .qwen2_blocks import Qwen2RMSNorm
from .qwen2_blocks import Qwen2Attention
from .qwen2_blocks import Qwen2RotaryEmbedding
from .qwen2_qblocks import W4A16Qwen2DecoderLayer
from .qwen2_uniql_qblocks import W4A16Qwen2UniQLDecoderLayer


@auto_docstring
class QQwen2PreTrainedModel(PreTrainedModel):
    config: Qwen2Config
...
@auto_docstring
class W4A16Qwen2Model(QQwen2PreTrainedModel):
    def __init__(self, config: Qwen2Config, **kwargs):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        block = W4A16Qwen2DecoderLayer
        if hasattr(config, "compress_config"):
            layer_ratio_config = kwargs.pop("layer_ratio_config", None)
            block = partial(W4A16Qwen2UniQLDecoderLayer, flex_layer_ratio=layer_ratio_config)
        self.embed_tokens = W4O16Embedding(config.vocab_size, config.hidden_size)
...
@auto_docstring
class W4A16Qwen2ForCausalLM(QQwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        super().__init__(config)
        self.model = W4A16Qwen2Model(config, **kwargs)
        self.vocab_size = config.vocab_size
    ...
    def from_pretrained(cls, pretrained_model_name, device=None, **kwargs):
        ...
        config = Qwen2Config(**config_data)
