...
class W4A16Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.v_proj = W4A16B16O16Linear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj = W4A16B16O16Linear(config.num_attention_heads * self.head_dim, config.hidden_size, bias=False)
...
class W4A16Qwen2DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = W4A16Qwen2Attention(config=config, layer_idx=layer_idx)
        self.mlp = W4A16Qwen2MLP(config, layer_idx=layer_idx)
