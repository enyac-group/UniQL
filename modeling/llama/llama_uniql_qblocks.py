...
class W4A16LlamaUniQLMLP(nn.Module):
...
class W4A16LlamaUniQLAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int, flex_layer_ratio=None):
        ...
        self.scaling = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)**-0.5 # use the original scaling
...
class W4A16LlamaUniQLDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int, flex_layer_ratio=None):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = W4A16LlamaUniQLAttention(config=config, layer_idx=layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
        self.mlp = W4A16LlamaUniQLMLP(config, layer_idx=layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
