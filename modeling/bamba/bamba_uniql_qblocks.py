...
class W4A16BambaUniQLMixer(nn.Module):
    """
...
class W4A16BambaUniQLAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_states, key_states = apply_rotary_pos_emb_indices(query_states, key_states, cos, sin, self.indices)
...
class W4A16BambaUniQLMLP(nn.Module):
...
class W4A16BambaUniQLDecoderLayer(nn.Module):
    ops = {
        "mlp": W4A16BambaUniQLMLP,
        "attention": W4A16BambaUniQLAttention,
        "mamba": W4A16BambaUniQLMixer,
    }
    ...
    def __init__(self, config: BambaConfig, layer_idx: int, layer_type: str = "mamba", flex_layer_ratio=None):
        ...
        if layer_type == "mamba":
            self.mamba = self.ops["mamba"](config=config, layer_idx=layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
        elif layer_type == "attention":
            self.self_attn = self.ops["attention"](config, layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
        else:
