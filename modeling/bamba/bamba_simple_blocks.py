...
from .bamba_rope import apply_rotary_pos_emb, apply_rotary_pos_emb_indices
...
class BambaMixerSimple(nn.Module):
    """
    ...
    def __init__(self, mamba_ssm, use_had_transform=False):
        ...
        # Split input projection into x, B, C, dt linear layers.
    ...
    def torch_forward(
        ...
        if hasattr(self, "in_proj"):
            ...
            gate = self.z_proj(input_states)
            # hidden_states_B_C = self.xBC_proj(hidden_states)
            x = self.x_proj(input_states)
            B = self.B_proj(input_states)
            C = self.C_proj(input_states)
            hidden_states_B_C = torch.cat([x, B, C], dim=-1)
            dt = self.dt_proj(input_states)
...
class BambaAttentionSimple(nn.Module):
...
class BambaMLPSimple(nn.Module):
