...
from .bamba_rope import apply_rotary_pos_emb_indices
from .bamba_blocks import apply_mask_to_padding_states
from .bamba_blocks import eager_attention_forward
from .bamba_blocks import pad_tensor_by_size, reshape_into_chunks, segment_sum
from .bamba_blocks import BambaRMSNormGated, BambaRMSNorm, HybridMambaAttentionDynamicCache
...
class BambaUniQLMixer(nn.Module):
    """
    ...
    def __init__(self, config: BambaConfig, layer_idx: int, flex_layer_ratio=None):
        super().__init__()
        ...
        if flex_layer_ratio is not None:
            self.ssm_state_size = 16 * int(round(config.mamba_d_state * flex_layer_ratio["mamba_ratio"] / 16.))
            self.head_dim = 2 * int(round(config.mamba_d_head * flex_layer_ratio["mamba_ratio"] / 2.))
        ...
        # self.head_dim = config.mamba_d_head # int(config.mamba_expand * self.hidden_size) // self.num_heads
        ...
        if flex_layer_ratio is not None:
            intermediate_size = int(config.mamba_expand * self.hidden_size)
    ...
    def cuda_kernels_masked_forward(
        ...
        if use_precomputed_states:
            ...
            hidden_states_B_C = causal_conv1d_update(
                hidden_states_B_C,
                cache_params.conv_states[self.layer_idx],
                self.conv1d.weight.squeeze(1) * conv_weight_mask,
...
class BambaUniQLAttention(nn.Module):
    ...
    def __init__(self, config: BambaConfig, layer_idx: int, flex_layer_ratio=None):
        ...
        # bamba only rotates half of the head_dim, see bamba_blocks.py apply_rotary_pos_emb function
        self.register_buffer("indices", torch.empty((config.num_key_value_heads, self.head_dim // 2), dtype=torch.uint8), persistent=True) # persistent to True so it is included in the module's state_dict for saving/loading model
    ...
    def _load_from_state_dict_flex_indices(self, state_dict, prefix, local_metadata, strict,
        ...
        for name, param in local_state.items():
            ...
            if "indices" in full_name:
                ...
                qk_head_dim = getattr(self.config, "head_dim", self.config.hidden_size // self.config.num_attention_heads)
                rot_dim = qk_head_dim // 4
                reduced_rot_dim = self.head_dim // 4
                loaded_indices = torch.cat([
                    loaded_param[:, :reduced_rot_dim],
                    loaded_param[:, rot_dim:rot_dim+reduced_rot_dim]], dim=-1)
...
class BambaUniQLMLP(nn.Module):
    def __init__(self, config, layer_idx: int, flex_layer_ratio=None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.hidden_size = config.hidden_size
...
class BambaUniQLDecoderLayer(nn.Module):

    def __init__(self, config: BambaConfig, layer_idx: int, layer_type: str = "mamba", flex_layer_ratio=None):
        super().__init__()
        self.config = config
        num_experts = 1
        ffn_layer_class = BambaUniQLMLP if num_experts == 1 else None
        self.feed_forward = ffn_layer_class(config, layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
        ...
        if layer_type == "mamba":
            self.mamba = BambaUniQLMixer(config=config, layer_idx=layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
        elif layer_type == "attention":
            self.self_attn = BambaUniQLAttention(config, layer_idx, flex_layer_ratio=flex_layer_ratio[layer_idx] if flex_layer_ratio is not None else None)
        else:
