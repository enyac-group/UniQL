...
__global__ void Marlin(
  ...
  const int4* __restrict__ B, // 4bit quantized weight matrix of shape kxn 
        int4* __restrict__ C, // fp16 global_reduce buffer of shape (max_par*16*4)xn
  ...
  bool output_transpose,             // whether to transpose the output, used in to_seqlen_last
  int tot_m,                         // total batch dimension
  ...
  if (slice_col_par >= n_tiles) {
    ...
    if (output_transpose) {
      D += ((slice_col_par / n_tiles) * 16 * thread_m_blocks) / 8;
    } else {
  ...
  auto write_result = [&] () {
    int d_gl_stride, d_gl_wr_delta, d_gl_wr, d_gl_wr_end, d_gl_wr_end_m;
    if (output_transpose) {
      d_gl_stride = tot_m;
      d_gl_wr_delta = 8 * threads / (2 * thread_n_blocks * 8);
      d_gl_wr = d_gl_stride * (threadIdx.x / (threads / (2 * thread_n_blocks * 8))) +
                  8 * (threadIdx.x % (threads / (2 * thread_n_blocks * 8)));
      d_gl_wr += d_gl_stride * (2 * thread_n_blocks * 8) * slice_col;
      d_gl_wr_end = d_gl_stride * prob_n;
      d_gl_wr_end_m = d_gl_stride * (2 * thread_n_blocks * 8) * slice_col
                        + d_gl_stride * (threadIdx.x / (threads / (2 * thread_n_blocks * 8)))
                        + prob_m;
    } else {
      d_gl_stride = prob_n / 8;
      d_gl_wr_delta = d_gl_stride * (threads / (2 * thread_n_blocks));
      d_gl_wr = d_gl_stride * (threadIdx.x / (2 * thread_n_blocks)) +
                    (threadIdx.x % (2 * thread_n_blocks));
      d_gl_wr += (2 * thread_n_blocks) * slice_col;
      d_gl_wr_end = d_gl_stride * prob_m;
    }
    ...
    int d_sh_rd, d_sh_rd_delta;
    if (output_transpose) {
      // We will read vertically in shared memory
      d_sh_rd_delta = 8 * d_sh_stride * (threads / (2 * thread_n_blocks));
      d_sh_rd = d_sh_stride * (threadIdx.x / (2 * thread_n_blocks * 8)) + (threadIdx.x % (2 * thread_n_blocks * 8));
    } else {
      d_sh_rd_delta = d_sh_stride * (threads / (2 * thread_n_blocks));
      d_sh_rd = d_sh_stride * (threadIdx.x / (2 * thread_n_blocks)) + (threadIdx.x % (2 * thread_n_blocks));
    }
  ...
  #pragma unroll
        for (int j = 0; j < 4; j++) {
          int wr = d_sh_wr + 8 * j;
          write(wr + (4 * d_sh_stride) * 0 + 0, frag_c[i][j][0][0], frag_c[i][j][0][1], frag_s[j / 2][2 * (j % 2) + 0]);
          write(wr + (4 * d_sh_stride) * 8 + 0, frag_c[i][j][0][2], frag_c[i][j][0][3], frag_s[j / 2][2 * (j % 2) + 0]);
          write(wr + (4 * d_sh_stride) * 0 + 4, frag_c[i][j][1][0], frag_c[i][j][1][1], frag_s[j / 2][2 * (j % 2) + 1]);
          write(wr + (4 * d_sh_stride) * 8 + 4, frag_c[i][j][1][2], frag_c[i][j][1][3], frag_s[j / 2][2 * (j % 2) + 1]);
        }
    ...
    for (int i = 0; i < ceildiv(16 * thread_m_blocks, threads / (2 * thread_n_blocks)); i++) {
      if (output_transpose) {
        if (d_gl_wr < d_gl_wr_end) {
          if (d_gl_wr + 8 < d_gl_wr_end_m) {
            int4 tmp;
            for (int j = 0; j < 8; j++) {
              reinterpret_cast<half*>(&tmp)[j] = *(reinterpret_cast<half*>(sh) + d_sh_rd + j*d_sh_stride*8);
            }
            D[d_gl_wr/8] = tmp; // this must align the memory address, so we have to pad total_m to be multiple of 8 (fp16)
          } else {
            for (int j = 0; d_gl_wr + j < d_gl_wr_end_m; j++) {
              *(reinterpret_cast<half*>(D) + d_gl_wr + j) = *(reinterpret_cast<half*>(sh) + d_sh_rd + j*d_sh_stride*8);
            }
          }
...
#define __CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, NUM_THREADS) \
    ...
    Marlin< \
      NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS \
    ><<<blocks, NUM_THREADS, SHARED_MEM, stream>>>( \
      A_ptr, B_ptr, C_ptr, D_ptr, s_ptr, \
      prob_m, prob_n, prob_k, \
      output_transpose, tot_m, locks \
    ); \
  }
...
int marlin_cuda(
  ...
  int tot_m = prob_m; // tot_m is the input prob_m
  ...
  //   group_blocks, blocks, cols);

  int err = 0;
  for (int i = 0; i < tot_m_blocks; i += 4) {
    int thread_m_blocks = tot_m_blocks - i;
    prob_m = tot_m - 16 * i; // tot_m is the input prob_m
    int par = 1;
    ...
    // CALL_IF(N_BLOCKS, K_BLOCKS, NUM_THREADS)
    // __CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, NUM_THREADS)
    if (false) {}
    CALL_IF(8, 8, 256)
    CALL_IF(16, 4, 256)
    // CALL_IF(8, 4, 128)  //  This will hang... not sure why
    // CALL_IF(4, 8, 128)  //  This will hang... not sure why
    else
      err = ERR_KERN_SHAPE;

    A_ptr += 16 * thread_m_blocks * (prob_k / 8) * par;
    if (output_transpose) {
      D_ptr += (16 * thread_m_blocks * par) / 8;
    } else {
...
torch::Tensor w4a16o16_gemm(
  ...
  auto options_c = torch::TensorOptions().dtype(torch::kFloat16).device(A.device());
  torch::Tensor C = torch::empty({max_par * 64, size_n}, options_c);

  // Alloc D matrix
  auto options_d = torch::TensorOptions().dtype(torch::kFloat16).device(A.device());
  torch::Tensor D;
  if (output_transpose) {
    size_m = ((size_m + 7) / 8) * 8; // Round up to nearest multiple of 8 for vactorized storing
    prob_m = size_m;
    D = torch::empty({size_n, size_m}, options_d); // to_seqlen_last
  } else {
    D = torch::empty({size_m, size_n}, options_d);
  }

  int err = W4A16O16::marlin_cuda(
    A.data_ptr(),
    B.data_ptr(),
    C.data_ptr(),
    D.data_ptr(),
  ...
  if (err != 0) {
    AT_ERROR("Failed to launch kernel.");
  }
  return D;
}