        ...
        # This is a hack to accommodate the huggingface's tokenizer API for testing mamba2-8b on the generation tasks
            ...
            if padding == "max_length":
                for i in range(len(encoding["input_ids"])):
                    if isinstance(encoding["input_ids"][i], list):
                        if self.padding_side == "left":
                            encoding["input_ids"][i] = [self._pad_id] * (max_length - encoding["input_ids"][i].shape[1]) + encoding["input_ids"][i]
                            encoding["attention_mask"][i] = [0] * (max_length - encoding["input_mask"][i].shape[1]) + encoding["attention_mask"][i]
                        elif self.padding_side == "right":
                            encoding["input_ids"][i] = encoding["input_ids"][i] + [self._pad_id] * (max_length - encoding["input_ids"][i].shape[1])
                            encoding["attention_mask"][i] = encoding["attention_mask"][i] + [0] * (max_length - encoding["input_mask"][i].shape[1])
                    elif isinstance(encoding["input_ids"][i], torch.Tensor):
                        if self.padding_side == "left":
                            encoding["input_ids"][i] = torch.cat([torch.ones(max_length - encoding["input_ids"][i].shape[1], dtype=torch.long).unsqueeze(0) * self._pad_id, encoding["input_ids"][i]], dim=1)
                            encoding["attention_mask"][i] = torch.cat([torch.ones(max_length - encoding["attention_mask"][i].shape[1], dtype=torch.long).unsqueeze(0) * 0, encoding["attention_mask"][i]], dim=1)
                        elif self.padding_side == "right":
                            encoding["input_ids"][i] = torch.cat([encoding["input_ids"][i], torch.ones(max_length - encoding["input_ids"][i].shape[1], dtype=torch.long).unsqueeze(0) * self._pad_id], dim=1)
                            encoding["attention_mask"][i] = torch.cat([encoding["attention_mask"][i], torch.ones(max_length - encoding["attention_mask"][i].shape[1], dtype=torch.long).unsqueeze(0) * 0], dim=1)
                    else:
                        raise ValueError(f"Unsupported type: {type(encoding['input_ids'][i])}")
            if isinstance(encoding["input_ids"], torch.Tensor):
...
if __name__ == '__main__':
    ...
    # tokenizer = _GPTSentencePieceTokenizer(model_file="/home/hc29225/QMT/pretrained_models/ut-enyac/mamba2-8b-converted/mt_nlg_plus_multilingual_ja_zh_the_stack_frac_015_256k.model")
    from modeling.build_models import build_model_and_tokenizer
    model, tokenizer = build_model_and_tokenizer(
        "ut-enyac/mamba2-8b-converted", pretrained_dir="pretrained_models", dtype=torch.bfloat16)
    truncation = False
    ...
    for i in range(len(encoding["input_ids"])):
        print(encoding["input_ids"][i].shape, encoding["attention_mask"][i].shape)
        break
    ...
    # from modeling.build_models import build_model_and_tokenizer
    # model, tokenizer = build_model_and_tokenizer("ut-enyac/mamba2-8b-converted", pretrained_dir="pretrained_models", dtype=torch.bfloat16)
    # # tokenizer = _GPTSentencePieceTokenizer(model_file="/mnt/dm-3/hychiang/pretrained_models/ut-enyac/mamba2-8b-converted/mt_nlg_plus_multilingual_ja_zh_the_stack_frac_015_256k.model")
    # print(tokenizer)
    # batch_size = 1
    # num_sample = 128
    # instruct_output_length = 2048
    # max_length = 2048
    # columns=["input_ids"]
    # torch_dataloader = True

    # from utils.dataset_utils import build_dataloader
    # dataloader, _ = build_dataloader(
    #     "yahma/alpaca-cleaned", tokenizer, batch_size=1,
    #     num_sample=128, max_length=2048,
    #     enable_instruct_prompting=False, instruct_output_length=2048,
    #     columns=["input_ids"])
